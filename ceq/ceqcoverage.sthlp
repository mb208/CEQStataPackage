{smcl}
{* oct112016}{...}
{cmd:help ceqcoverage} (beta version; please report bugs) {right:Sean Higgins}
{hline}

{title:Title}

{p 4 11 2}
{hi:ceqcoverage} {hline 2} Computes indicators on coverage and leakages for fiscal 
interventions (taxes, transfers, and subsidies) by income group for the "E18. Coverage Tables" sheets of the CEQ Master Workbook 2016 Section E

{title:Syntax}

{p 8 11 2}
    {cmd:ceqcoverage} {ifin} {weight} [{cmd:using} {it:filename}] [{cmd:,} {it:options}]{break}
...
{synopt :{opth recp:ensions(varlist)}}Contributory pension variables{p_end}
{synopt :{opth recdtr:ansfers(varlist)}}Direct transfer variables{p_end}
{synopt :{opth paydtax:es(varlist)}}Direct tax variables{p_end}
{synopt :{opth payco:ntribs(varlist)}}Contribution variables{p_end}
{synopt :{opth recsu:bsidies(varlist)}}Subsidy variables{p_end}
{synopt :{opth payindtax:es(varlist)}}Indirect tax variables{p_end}
{synopt :{opth rechealth(varlist)}}Health variables{p_end}
{synopt :{opth receduc:ation(varlist)}}Education variables{p_end}
{synopt :{opth recother:public(varlist)}}Other public in-kind transfers{p_end}
{synopt :{opth payuserfeesh:ealth(varlist)}}Health user fees variables{p_end}
{synopt :{opth payuserfeese:duc(varlist)}}Education user fees variables{p_end}
{synopt :{opth payuserfeeso:ther(varlist)}}Other public user fees variables{p_end}
...
{pstd} 
{cmd:ceqcoverage} calculates coverage and leakage indicators by income group for 
...
{cmd:ceqcoverage} produces one sheet for each of the CEQ core income concepts; the income concept 
...
{pstd}
{cmd: ceqcoverage} automatically converts local currency variables to PPP dollars, using the PPP conversion 
...
using {help svyset} since {cmd:ceqcoverage} automatically uses the information specified using {help svyset}. 
...
{pstd}
By default, {cmd: ceqcoverage} does not allow income concept or fiscal intervention variables to have missing 
...
default, {cmd:ceqcoverage} prints to the sheets titled "E18.X Coverage Tables" where X indicates the 
...
Workbook populated with results from {cmd:ceqcoverage} can be automatically opened if the {opt open} 
...
{phang}
Higgins, Sean and Nora Lustig. 2016. {browse "http://www.sciencedirect.com/science/article/pii/S0304387816300220":"Can a Poverty-Reducing and Progressive Tax and Transfer System Hurt the Poor?"} {it:Journal of Development Economics} 122, 63-75.{p_end}
