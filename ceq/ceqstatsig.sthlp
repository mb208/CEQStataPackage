{smcl}
{* 25aug2016}{...}
{cmd:help ceqstatsig} (beta version; please report bugs) {right:Sean Higgins}
{hline}

{title:Title}

{p 4 11 2}
{hi:ceqstatsig} {hline 2} Statistical significance tests of the change in inequality and poverty between income concepts for sheet "E7. Statistical Significance" of the CEQ Master Workbook 2016

{title:Syntax}

{p 8 11 2}
    {cmd:ceqstatsig} {ifin} {weight} [{cmd:using} {it:filename}] [{cmd:,} {it:options}]{break}
...
{synopt :{opth sheet(string)}}Name of sheet to write population matrices. Default is "E7. Statistical Significance"{p_end}
...
and {opth pl3(real)} options; the defaults for these are the commonly-used $1.25, $2.50, and $4 
PPP poverty lines. For example, to change the lowest poverty line from $1.25 PPP per day to 
$1.90 PPP per day, specify {cmd:pl1(1.90)}. Poverty lines in local currency can be entered using 
the {opth nationale:xtremepl(string)} and {opth nationalm:oderatepl(string)} options. Local currency poverty 
lines can be entered as real numbers (for poverty lines that are fixed for the entire 
population) or variable names (for poverty lines that vary, for example across space), and 
should be in the same units as the income concept variables (preferably local currency units per 
...
proportion is 0.5, i.e. 50% of household median income. For example, to change the relative poverty line 
from 50% to 60% of median income (which is used by the OECD), specify {cmd:proportion(0.6)}.

{pstd}
{cmd: ceqstatsig} automatically converts local currency variables to PPP dollars, using the PPP conversion 
...
{pstd}
If the data set is at the individual level (each observation is an individual), the variable 
with the identification code of each household (i.e., it takes the same value for all members 
within a household) should be specified in the {opth hh:id(varname)} option; the 
{opth hs:ize(varname)} option should not be specified. If the data set is at the household 
level, the number of members in the household should be specified in {opth hs:ize(varname)}; the 
{opth hh:id(varname)} option should not be specified. In either case, the weight used should be 
the household sampling weight and should {it:not} be multiplied by the number of members in the 
household since the program will do this multiplication automatically in the case of 
...
using {help svyset} since {cmd:ceqstatsig} automatically uses the information specified using 
...
{pstd}
By default, {cmd: ceqstatsig} does not allow income concept or fiscal intervention variables to have missing 
...
{cmd:using} {it:filename} is specifed in the command, where {it:filename} is the Master 
Workbook. By default, {cmd:ceqstatsig} prints to a sheet titled "E7. Statistical Significance"; 
the user can override the sheet name using the {opt sheet(string)} option. Exporting directly to 
the Master Workbook requires Stata 13 or newer. The Master Workbook populated with results from 
{cmd:ceqstatsig} can be automatically opened if the {opt open} option is specified (in this 
case, {it:filename} cannot have spaces). 
...
{phang} {cmd:. ceqstatsig [pw=w] using C:/Output_Tables.xlsx, hhid(hh_code) psu(psu_var) strata(stra_var) m(ym) mplusp(ymplusp) n(yn) g(yg) t(yt) d(yd) c(yc) f(yf) ppp(`ppp') cpibase(`cpi05') cpisurvey(`cpi') nationale(PLipea_ext) nationalm(PLipea) open}{p_end}

{pstd}Household-level data (each observation is a household){p_end}
{phang} {cmd:. ceqstatsig [pw=w] using C:/Output_Tables.xlsx, hsize(members) psu(psu_var) strata(stra_var) m(ym) mp(ymplusp) n(yn) g(yg) t(yt) d(yd) c(yc) f(yf) ppp(`ppp') cpibase(`cpi05') cpisurvey(`cpi') nationale(PLipea_ext) nationalm(PLipea) open}{p_end}
