{smcl}
{* 19aug2016}{...}
{cmd:help ceqconc} (beta version; please report bugs) {right:Sean Higgins}
{hline}

{title:Title}

{p 4 11 2}
{hi:ceqconc} {hline 2} Computes non-anonymous summary statistics and detailed information by income decile, group, centile, and bin for the CEQ core income concepts for the "E10. Concentration" sheets of the CEQ Master Workbook 2016 Section E

{title:Syntax}

{p 8 11 2}
    {cmd:ceqconc} {ifin} {weight} [{cmd:using} {it:filename}] [{cmd:,} {it:options}]{break}
...
{synopt :{opth sheetn(string)}}Name of sheet to write results ranking by net market income. Default is "E10.n Concentration"{p_end}
{synopt :{opth sheetg(string)}}Name of sheet to write results ranking by gross income. Default is "E10.g Concentration"{p_end}
{synopt :{opth sheett(string)}}Name of sheet to write results ranking by taxable income. Default is "E10.t Concentration"{p_end}
{synopt :{opth sheetd(string)}}Name of sheet to write results ranking by disposable income. Default is "E10.d Concentration"{p_end}
{synopt :{opth sheetc(string)}}Name of sheet to write results ranking by consumable income. Default is "E10.c Concentration"{p_end}
{synopt :{opth sheetf(string)}}Name of sheet to write results ranking by final income. Default is "E10.f Concentration"{p_end}
...
{pstd} 
{cmd:ceqconc} requires installation of {cmd:quantiles} (Osorio, 2007); to install, {stata ssc install quantiles:ssc install quantiles}.

{title:Description}

{pstd} 
{cmd:ceqconc} calculates non-anonymous summary statistics and detailed information by decile, income 
group, centile, and income bin for each of the CEQ core income concepts, where deciles, groups, 
centiles, and bins are defined holding the income concept fixed within each sheet. Hence, 
{cmd:ceqconc} produces one sheet for each of the CEQ core income concepts; the income concept defining 
the ranking of each sheet will be referred to in this help file as the ranking variable. Summary 
statistics include the mean, median, standard deviation, and a number of measures for each core income 
concept with respect to the ranking variable: its concentration coefficient, redistributive effect, 
Reynolds-Smolensky (or vertical equity), and reranking effect. The detailed information by decile, 
income group, centile, and income bin includes total income in local currency units (preferably per 
year) and PPP dollars per day, per capita income in local currency (preferably per year) and PPP 
dollars per day, concentration shares, cumulative concentration shares, and fiscal incidence with 
respect to the ranking variable.
...
{pstd}
These sheets differ from the Lorenz sheet because in these sheets, the ranking of household is held 
fixed across columns within each sheet. In other words, the results are non-anonymous: we define 
deciles, groups, centiles, and bins once for each sheet. The CEQ core income concepts include market 
income, market income plus pensions, net market income, gross income, taxable income, disposable 
income, consumable income, and final income. The variables for these income concepts, which should be 
expressed in local currency units (preferably {bf:per year} for ease of comparison with totals from 
national accounts), are indicated using the {opth m:arket(varname)}, {opth mp:luspensions(varname)}, 
{opth n:etmarket(varname)}, {opth g:ross(varname)}, {opth t:axable(varname)}, 
{opth d:isposable(varname)}, {opth c:onsumable(varname)}, {opth c:onsumable(varname)}, and 
{opth f:inal(varname)} options. 

{pstd}
{cmd: ceqconc} automatically converts local currency variables to PPP dollars, using the PPP conversion 
...
using {help svyset} since {cmd:ceqconc} automatically uses the information specified using {help svyset}. 
...
{pstd}
By default, {cmd: ceqconc} does not allow income concept or fiscal intervention variables to have missing 
...
default, {cmd:ceqconc} prints to the sheets titled "E10.X Concentration" where X indicates the income 
...
Workbook populated with results from {cmd:ceqconc} can be automatically opened if the {opt open} 
option is specified (in this case, {it:filename} cannot have spaces). To produce only a portion of the 
results, specify only a subset of the income concept options or use {opt nod:ecile}, {opt nog:roup}, 
{opt noc:entile}, or {opt nob:in}.
...
{phang} {cmd:. ceqconc [pw=w] using C:/Output_Tables.xlsx, hhid(hh_code) psu(psu_var) strata(stra_var) m(ym) mplusp(ymplusp) n(yn) g(yg) t(yt) d(yd) c(yc) f(yf) ppp(`ppp') cpibase(`cpi05') cpisurvey(`cpi') open}{p_end}

{pstd}Household-level data (each observation is a household){p_end}
{phang} {cmd:. ceqconc [pw=w] using C:/Output_Tables.xlsx, hsize(members) psu(psu_var) strata(stra_var) m(ym) mp(ymplusp) n(yn) g(yg) t(yt) d(yd) c(yc) f(yf) ppp(`ppp') cpibase(`cpi05') cpisurvey(`cpi') open}{p_end}
