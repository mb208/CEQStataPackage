...
{p 4 11 2}
{hi:ceqextsig} {hline 2} Statistical significance tests of the change in inequality and poverty between core and extended income concepts for sheet "E16. Extended Income Stat Sig" of the CEQ Master Workbook 2016

{title:Syntax}

{p 8 11 2}
    {cmd:ceqextsig} {ifin} {weight} [{cmd:using} {it:filename}] [{cmd:,} {it:options}]{break}
...
{synopt :{opth sheet(string)}}Name of sheet to write population matrices. Default is "E16. Extended Income Stat Sig"{p_end}
...
{pstd}
{cmd: ceqextsig} automatically converts local currency variables to PPP dollars, using the PPP conversion 
...
using {help svyset} since {cmd:ceqextsig} automatically uses the information specified using 
...
{pstd}
By default, {cmd: ceqextsig} does not allow income concept or fiscal intervention variables to have missing 
...
Workbook. By default, {cmd:ceqextsig} prints to a sheet titled "E16. Extended Income Stat Sig"; 
...
{cmd:ceqextsig} can be automatically opened if the {opt open} option is specified (in this 
...
{phang} {cmd:. ceqextsig [pw=w] using C:/Output_Tables.xlsx, hhid(hh_code) psu(psu_var) strata(stra_var) m(ym) mplusp(ymplusp) n(yn) g(yg) t(yt) d(yd) c(yc) f(yf) ppp(`ppp') cpibase(`cpi05') cpisurvey(`cpi') nationale(PLipea_ext) nationalm(PLipea) open}{p_end}

{pstd}Household-level data (each observation is a household){p_end}
{phang} {cmd:. ceqextsig [pw=w] using C:/Output_Tables.xlsx, hsize(members) psu(psu_var) strata(stra_var) m(ym) mp(ymplusp) n(yn) g(yg) t(yt) d(yd) c(yc) f(yf) ppp(`ppp') cpibase(`cpi05') cpisurvey(`cpi') nationale(PLipea_ext) nationalm(PLipea) open}{p_end}
