{smcl}
{* 21aug2016}{...}
{cmd:help ceqfiscal} (beta version; please report bugs) {right:Sean Higgins}
{hline}

{title:Title}

{p 4 11 2}
{hi:ceqfiscal} {hline 2} Computes summary statistics and detailed information for fiscal interventions (taxes, transfers, and subsidies) by income decile, group, centile, and bin for the "E11. FiscalInterventions" sheets of the CEQ Master Workbook 2016 Section E

{title:Syntax}

{p 8 11 2}
    {cmd:ceqfiscal} {ifin} {weight} [{cmd:using} {it:filename}] [{cmd:,} {it:options}]{break}
...
{pstd} 
{cmd:ceqfiscal} requires installation of {cmd:quantiles} (Osorio, 2007); to install, {stata ssc install quantiles:ssc install quantiles}.

{title:Description}

{pstd} 
{cmd:ceqfiscal} calculates summary statistics and detailed information by decile, income group, 
centile, and income bin for fiscal interventions (taxes, transfers, and subsidies), where deciles, 
groups, centiles, and bins are defined holding the income concept fixed within each sheet. Hence, 
{cmd:ceqfiscal} produces one sheet for each of the CEQ core income concepts; the income concept 
defining the ranking of each sheet will be referred to in this help file as the ranking variable. 
Summary statistics include the mean, median, standard deviation, and measures for each fiscal 
intervention with respect to the ranking variable: its concentration coefficient and Kakwani 
coefficient. The detailed information by decile, income group, centile, and income bin includes--for 
each fiscal intervention--total received or paid in local currency units (preferably per year) and PPP 
dollars per day, per capita amount received or paid in local currency (preferably per year) and PPP 
...
{pstd}
{cmd: ceqfiscal} automatically converts local currency variables to PPP dollars, using the PPP conversion 
...
using {help svyset} since {cmd:ceqfiscal} automatically uses the information specified using {help svyset}. 
...
{pstd}
By default, {cmd: ceqfiscal} does not allow income concept or fiscal intervention variables to have missing 
...
default, {cmd:ceqfiscal} prints to the sheets titled "E11.X FiscalInterventions" where X indicates the 
...
Workbook populated with results from {cmd:ceqfiscal} can be automatically opened if the {opt open} 
