{smcl}
{* 8aug2016}{...}
{cmd:help ceqdes} (beta version; please report bugs) {right:Sean Higgins}
{hline}

{title:Title}

{p 4 11 2}
{hi:ceqdes} {hline 2} Computes descriptive statistics for the CEQ core income concepts and fiscal interventions, for the "E1. Descriptive Statistics" sheet of the CEQ Master Workbook 2016 Section E

{title:Syntax}

{p 8 11 2}
    {cmd:ceqdes} {ifin} {weight} [{cmd:using} {it:filename}] [{cmd:,} {it:options}]{break}
...
{synopt :{opth sheet(string)}}Name of sheet to write results. Default is "E1. Descriptive Statistics"{p_end}
...
{pstd} 
{cmd:ceqdes} calculates descriptive statistics for the CEQ core income concepts and fiscal 
interventions (taxes, transfers, and subsidies). The descriptive statistics are the percent of 
individuals in the expanded sample that have positive values for the income concept or non-zero values 
for the fiscal intervention variable, among those with positive or non-zero values, the mean, median, 
and standard error of the variable, and among all individuals, the total for that variable as a 
proportion of total final income.
...
{pstd}
*** The income and fiscal intervention variables should be expressed in household per capita or per adult equivalent terms, regardless of whether the data set being used is at the household or individual level. Hence, they should have the same per-person 
amount for each member within a household when using individual-level data. ***
...
using {help svyset} since {cmd:ceqdes} automatically uses the information specified using {help svyset}. 
...
{pstd}
By default, {cmd: ceqdes} does not allow income concept or fiscal intervention variables to have missing 
...
{cmd:ceqdes} prints to a sheet titled "E1. Descriptive Statistics"; the user can override the sheet 
name using the {opt sheet(string)} option. Exporting directly to the Master Workbook requires Stata 13 
or newer. The Master Workbook populated with results from {cmd:ceqdes} can be automatically opened if 
