...
** ceqindchar PROGRAM **
**********************
** For sheet E23. IndivSociodemoCharac
// BEGIN ceqindchar (Higgins 2016)
capture program drop ceqindchar
program define ceqindchar, rclass 
	version 13.0
	#delimit ;
	...
	** collapse to hh-level data 
	if "`hsize'"=="" { // i.e., it is individual-level data
		...
		foreach char of local characteristics {
			...
			qui replace `char' = `mean_char' // hh-level averages
		}
	...
	** ado file specific
	foreach vrank of local alllist {
		if "`sheet`vrank''"=="" {
			...
			local sheet`vrank' "E23.`_vrank' IndivSociodemoCharac" // default name of sheet in Excel files
		}
	}
